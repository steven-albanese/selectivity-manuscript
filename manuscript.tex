...
\caption{
\label{fig:figure-1}
{\bf Absolute free energy calculations show a wide range of errors in predicting selectivity} \\
({\bf A})  A representative structure of BRD2 (4ALG) bound to ligand RVX-OH, highlighting the binding mode of the ligands considered in this paper. 
({\bf B}) The chemical structure of all three brodomain inhibitors used in this reference. 
({\bf C}) he bromodomain family tree, adapted from Aldeghi et al., 2016. Bromodomains shown in green (from original) had absolute free energy calculations performed on them. The orange box highlights the bromodomains for which RVX-208, RXV-OH and Bromosporine calculations were performed. The blue box contains bromodomains for which only Bromosporine calculations were performed. 
({\bf D})The calculated $\Delta\Delta$G for each possible pairing of bromodomains. Orange ellipses represent a pair in which one or both of the bromodomains are from Family II (orange box, 1C). The blue ellipses contain only bromodomains from outside family II (blue box, 1C). The radii of the ellipses show the statistical uncertainty (in the Y-axis) or the experimental uncertainty (in the X-axis). The solid black line corresponds to absolute agreement between predicted $\Delta\Delta$G (Y-axis) and experiment $\Delta\Delta$G (X-axis). The Mean Unsigned Error (MUE) and Root Mean Square Error (RMSE) were calculated and shown with bootstrapped 95$\%$ confidence intervals, as described in the methods section.
}
\end{fullwidth}
\end{figure}
...
\todo{figure out why figure4 is rendering this way}
\begin{figure}
\begin{fullwidth}
\begin{centering}
